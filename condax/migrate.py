...
def move_condax_config() -> None:
    ...
    if from_.exists():
        ...
        if to_.exists():
            logging.info(f"A file already exists at {to_}; skipping to handle the old config file at {from_}")
            return
        ...
        for subdir in from_.iterdir():
            ...
            if subdir.is_dir():
                if not dst.exists():
                    ...
                    print(f"  [migrate] Skipping {subdir} as it already exists at {dst}")
